...
public class WopiHostContrller {
	...
	@ResponseBody
	public void getFileInfo(@PathVariable(name = "name") String name, HttpServletRequest request,
		...
		if (access_token == null || access_token.length() < 1)
			return;
		String[] tokens = access_token.split("_");
		...
		info.setUserFriendlyName(userid);
		try {
			// 获取文件名
